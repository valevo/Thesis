...
def get_length_matched(sents, k):
    ...
    return tuple(len_matched_sents)
...
def parallel_filtering(args):
    dir_to_save, cur_j, *filter_args = args
    ...
    print("ID", id(length_matched_sents), "PID", os.getpid())
    rand.seed(cur_seed)
...
#        j_start_at = 0
    ...
    if FilterCls is SpeakerRestrictionFilterRandomised:
        params_combined = [(top_dir+save_dir, j, n, m, "count_tokens") for n in ns 
