...
def prepare_filter_training(subword_model_dir, filter_obj):
    translator = SentencePieceTranslator(subword_model_dir)
...
#    cur_m = 100
    ...
    dir_prefix, subword_dir, filter_dir, log_dir, weight_dir = prepare_folders(lang)
    
    print("set up directories...")
    
    # CHANGED
    filters = get_filters(filter_dir, lambda f: True, 
                          to_list=True)
    ...
    for i, cur_f in enumerate(filters):        
        ...
        f_gen = prepare_filter_training(subword_dir, cur_f)
